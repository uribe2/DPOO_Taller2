...
public class SandboxListas
{
    ...
    {
    	List<String> copia = new LinkedList<>(listaCadenas);
        return copia;
    }
    ...
    public void eliminarCadena( String cadena )
    {
        ...
        while (iterator.hasNext()) {
            if (iterator.next().equalsIgnoreCase(cadena)) {
                iterator.remove();
            }
        }
    }
    ...
    public void reiniciarArregloEnteros( double[] valores )
    {
    	...
    	for (int i = 0; i < valores.length; i++) {
    		int trunc = (int) valores[i];
    		listaEnteros.add(trunc);
    	}
    }
    ...
    public int contarEnterosRepetidos( )
    {
        ...
        for (Integer entero : listaEnteros) {
            conteo.put(entero, conteo.getOrDefault(entero, 0) + 1);
        }
        int repetidos = 0;
        for (Integer cantidad : conteo.values()) {
            if (cantidad > 1) {
                repetidos++;
            }
        }
